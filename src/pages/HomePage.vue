...
onMounted(async () => {
  ...
  try {
    ...
    announcements.value = anns.filter(
      (a) => !isAnnouncementDismissed(a.id, a.reactivated_at || null),
    )
...
<template>
  <div>
    <h1 class="text-2xl font-bold mb-6" style="color: var(--color-text)">
        ...
        >
          <div class="flex justify-between items-start">
            <div class="flex-1">
              ...
              <div
                class="text-sm mt-1 prose prose-sm max-w-none"
                style="color: var(--color-text-muted)"
                v-html="a.content"
              ></div>
              <div
                class="flex items-center space-x-3 mt-2 text-xs"
                style="color: var(--color-text-muted)"
              >
    ...
    <div class="rounded-lg shadow p-6 mb-6" style="background-color: var(--color-surface)">
      <div class="flex justify-between items-center mb-4">
        ...
        <router-link to="/my-bookings" class="text-sm" style="color: var(--color-primary)"
        ...
        >
          <div>
            ...
            <p class="text-sm capitalize" style="color: var(--color-text-muted)">
              {{ formatDate(b.date) }} · {{ formatTime(b.services?.start_time) }} -
              {{ formatTime(b.services?.end_time) }}
